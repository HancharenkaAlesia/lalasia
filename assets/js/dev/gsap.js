...
function gsapInit() {
  const chars = document.querySelectorAll("[data-fls-gsap]");
  const charsOpacity = document.querySelectorAll("[data-fls-gsap-opacity]");
  gsapWithCSS.registerPlugin(ScrollTrigger);
  chars.forEach((char) => {
    gsapWithCSS.from(char, {
      scrollTrigger: char,
      opacity: 0,
      y: 100,
      duration: 1,
  ...
  charsOpacity.forEach((charOp) => {
    gsapWithCSS.from(charOp, {
      scrollTrigger: charOp,
      start: "middle bottom",
      opacity: 0,
      scrub: 1,
      duration: 1.2
...
document.querySelector("[data-fls-gsap]") || document.querySelector("[data-fls-gsap-opacity]") ? window.addEventListener("load", gsapInit) : null;
